...
class DQNLightning(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Current Q values
        q_values = torch.gather(self.net(state_batch), 1, action_batch.long().unsqueeze(-1))
        
        # Target Q values
        with torch.no_grad():
            new_state_values = torch.zeros(self.batch_size, device=self.device, dtype=torch.float)
            new_state_values[~done_batch] = torch.max(self.target_net(new_state_batch[~done_batch]), dim=1)[0]
            expected_q_values = reward_batch + (self.gamma * new_state_values * (~done_batch))
...
class ReplayBuffer(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.tensor(batch.state), # 원래 FloatTensor로 state를 받았을때는 stack 이였음
            torch.tensor(batch.action),
            torch.tensor(batch.reward),
            torch.tensor(batch.new_state),# 원래 FloatTensor로 state를 받았을때는 stack 이였음
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(project=f"lightning_dqn_frozenlake")
    model = DQNLightning()
