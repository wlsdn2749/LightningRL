...
def run():
    ...
    for idx in range(episodes):
        ...
        state = torch.FloatTensor(
            one_hot_encode(state, state_size)
        )  # .unsqueeze(0) # shape = [1x64]
        ...
        while not terminated and not truncated:
            ...
            new_state = torch.FloatTensor(
                one_hot_encode(new_state, state_size)
            )  # .unsqueeze(0) # shape = [1x64]
            ...
            if len(D) >= batch_size:
                ...
                with torch.no_grad():
                    new_state_values[~done_batch] = torch.max(
                        Q(new_state_batch[~done_batch]), dim=1
                    )[0]  # [32]

                expected_q_values = reward_batch + (
                    reward_factor * new_state_values * (~done_batch)
                )
        ...
        if idx % 1000 == 0:
            print(
                f"Reward mean ~{idx} : {torch.mean(reward_per_episodes[idx-1000:idx])}"
            )
